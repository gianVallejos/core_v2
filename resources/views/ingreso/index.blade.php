                    ...
                    <form class="form-horizontal" action="/core_v2/ingresos" method="POST">
                            ...
                            <div class="col-md-4 col-xs-5">
                                <select name="paciente_id" id="paciente_id" class="form-control">
                                    @foreach( $pacientes as $stdo )
                                        <option value="{{ $stdo->id }}")selected>{{ $stdo->id }} - {{ $stdo->apellidos }} {{ $stdo->nombres }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4 col-xs-5">
                                <select name="doctor_id" id="doctor_id" class="form-control">
                                    @foreach( $medicos as $stdo )
                                        <option value="{{ $stdo->id }}")selected>{{ $stdo->apellidos }} {{ $stdo->nombres }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6 col-xs-8">
                                <input id="descripcion" type="text" class="form-control" name="descripcion" value="{{ old('descripcion')}}" placeholder="Descripcion">
                            ...
                            <div class="col-md-2 col-xs-2">
                                <input id="monto" type="number" class="form-control" name="monto" value="{{ old('monto')}}" placeholder="Monto" min="0" step=".1" required>
                      ...
                      <div class="row">
                        <form id="form-buscar-ingreso" method="get">
                            ...
                            <div class="col-md-3 col-xs-4">
                              <input id="date_inicio" type="date" name="date_inicio" value="" class="form-control">
                            </div>
                            <div class="col-md-3 col-xs-4">
                              <input id="date_fin" type="date" name="date_fin" value="" class="form-control">
                            </div>
                            <div class="col-md-4 col-xs-4">
                              <select name="doctor_id" id="doctor_id" class="form-control">
                                  <option value="-1" selected>Todos los Doctores</option>
                                  @foreach( $medicos as $stdo )
                                      <option value="{{ $stdo->id }}")>{{ $stdo->apellidos }} {{ $stdo->nombres }}</option>
                                  @endforeach
                              </select>
                            </div>
                            ...
                            <tbody>
                                ...
                                @foreach( $ingresos as $row )
                                  <tr>
                                    ...
                                    <td class="text-center"><button class="btn btn-xs btn-success" onclick="mostrarDetalleIngreso('{{ json_encode($ingresos[$i]) }}')" data-toggle="modal" data-target="#myModal">Detalle</button></td>
                                    <td class="text-center"><a href="{{ route('ingresos.edit', $row->id) }}" class="btn btn-xs btn-warning">Editar</a></td>
                                    <td class="text-center">
                                      <form action="{{ route('ingresos.destroy', $row->id) }}" method="post">
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="row">
          <label for="montoDoctor" class="col-md-2 col-md-offset-1 control-label">Monto Doctor:</label>
